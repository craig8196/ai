...
class TeamManager(object):
    ...
    def __init__(self, bzrc):
        self.bzrc = bzrc
        self.beliefs = self.bzrc.get_environment_beliefs()
        self.tanks = []
        # Create all tanks
        for i in range(0, int(self.beliefs.get_count(self.beliefs.color))):
            self.tanks.append(PFieldTank(i, self.bzrc, self.beliefs))
        # Put each tank on a thread
        for tank in self.tanks:
            tank.setDaemon(True)
            tank.start()
        # Create Kalman Filters for all enemy tanks
        self.beliefs.init_kalman_filters()
    ...
    def tick(self, time_diff):
        # Get a new state
        env_state = self.bzrc.get_environment_state(self.beliefs.color)
        env_state.time_diff = time_diff
        
        # Update enemy locations
        self.beliefs.update_kalman_filters(env_state)
...
class FinalTank(Thread):
    ...
    def initialize_states(self):
        ...
        self.color = self.beliefs.color

        for base in self.beliefs.bases:
            if base.color == self.color:
                self.our_base = base
        
        self.ourbase_center_x, self.ourbase_center_y = self.beliefs.get_base_location(self.beliefs.color)
    ...
    def init_behaviors(self):
        ...
        for i, enemyflag in enumerate(self.env_state.enemyflags):
            self.behaviors.append(SeekGoalBehavior("flag" + enemyflag.color, self.mytank, 170, 2, 0.5, self.beliefs.color, i))

        #create the behavior for bringing a captured flag back to our base
        self.behaviors.append(SeekGoalBehavior("base", self.mytank, 400, 5, 0.5, self.beliefs.color, -1))
